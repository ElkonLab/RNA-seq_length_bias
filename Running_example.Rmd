...
The counts file and the Tx.len.file should be located in Counts_data folder within the working directory.
The output file is cnts_data_Pgenes.txt


```{r filter genes}
setwd("/home/elkon/zoharmanber/length_bias/")
...
identify_genes_with_leninfo_and_cpm_above_thresh (countsFile, Conds,N.samples.cutoff=N.replicates, CPM.CUTOFF=1.0, Tx.LenFile, DatasetTitle=DatasetTitle) 
```
...
```{r Step 2: compare biological conditions}
countsFile.Pgenes <- ("TNFa_GSE64233_cnts_data_Pgenes.txt")
length_bias_analysis_compare_biological_conditions(countsFile.Pgenes, Tx.LenFile, DatasetTitle=DatasetTitle, X.lim=c(8, 15), Y.lim=c(-2,2)) 
```
...
```{r Step 3: compare replicate samples (on treatment replicates)}
Rep.first.col <- N.replicates + 2
length_bias_analysis_compare_replicate_samples(countsFile.Pgenes, Tx.LenFile,N.replicates=N.replicates, DatasetTitle=DatasetTitle, Rep.first.col=Rep.first.col, X.lim=c(8, 15), Y.lim=c(-3,3)) 
```
...
```{r Step 4: compare biological and replicate samples (on treatment replicates) after cqn normalization,echo = FALSE}

length_bias_analysis_cqn_normalization (countsFile.Pgenes,Tx.LenFile,N.replicates=N.replicates, DatasetTitle=DatasetTitle, Rep.first.col=Rep.first.col, X.lim=c(8, 15), Y.lim=c(-4,4)) 
