...
library(cqn)
# 
# DE analysis  - dataset with replicates: one factor design - TWO biological conditions 
#
...
#
plotFCvsLen <- function(logTxLen, logFC, main.title,xLab="Tx.length (log2)", X.lim, Y.lim, cex.Main=1.0) {
...
# with equal number of replicates, ordered in the input files such that 1st col is Sym, then cols 2:(ncols/2) are for cond1 and (ncols/2)+1:ncols for cond2.  
#
...
length_bias_analysis_compare_biological_conditions <- function (countsFile.Pgenes, Hs.Tx.LenFile, DatasetTitle="TNFa.p65kd", X.lim=c(8, 15), Y.lim=c(-2,2)) {
  ...
  RPKM.logFC <- calcLogFC(RPKM,Conds)#log( apply(RPKM[, Conds[[2]]], 1, mean) / apply(RPKM[, Conds[[1]]], 1, mean), 2)
  
  main.title <- sprintf("%s: RPKM", DatasetTitle);
  r <- plotFCvsLen(logTxLen=log(dge$genes[, "Tx.len"],2), logFC=RPKM.logFC, main.title,xLab="Tx.length (log2)",X.lim, Y.lim)
  P.bio[1, 1]          <- r$p.value; 
  ...
  r <- plotFCvsLen(logTxLen=log(dge$genes[, "Tx.len"],2), logFC=RPKM.qnorm.logFC, main.title,xLab="Tx.length (log2)", X.lim, Y.lim)
  P.bio[1, 2]          <- r$p.value; 
  ...
  r <- plotFCvsLen(logTxLen=log(dge$genes[, "Tx.len"],2), logFC=edgeR.TMM.logFC, main.title,xLab="Tx.length (log2)", X.lim, Y.lim)
  P.bio[1, 3]          <- r$p.value; 
  ...
  r <- plotFCvsLen(logTxLen=log(dge$genes[, "Tx.len"],2), logFC=edgeR.RLE.logFC, main.title,xLab="Tx.length (log2)", X.lim, Y.lim)
  P.bio[1, 4]          <- r$p.value; 
  ...
  r <- plotFCvsLen(logTxLen=log(dge$genes[, "Tx.len"],2), logFC=RPKM.rle.logFC, main.title,xLab="Tx.length (log2)", X.lim, Y.lim)
  P.bio[1, 5]          <- r$p.value; 
  ...
  r <- plotFCvsLen(logTxLen=log(dge$genes[, "Tx.len"],2), logFC=RPKM.uq.logFC, main.title,xLab="Tx.length (log2)", X.lim, Y.lim)
  P.bio[1, 6]          <- r$p.value; 
  ...
  M.FC <- cbind(dge.uq$genes[, "sym"], RPKM.logFC, RPKM.qnorm.logFC, edgeR.TMM.logFC, edgeR.RLE.logFC, RPKM.rle.logFC, RPKM.uq.logFC)
  colnames(M.FC) <- c("sym", "RPKM.logFC", "RPKM.qnorm.logFC", "edgeR.TMM.logFC", "edgeR.RLE.logFC", "RPKM.rle.logFC", "RPKM.uq.logFC")
  outFile <- sprintf("%s_treatment_vs_control_analysis_logFC.txt", DatasetTitle);
  write.table(M.FC, file=outFile, sep="\t", quote=F, col.names=NA)
  str <- sprintf("\nlogFC results recorded in %s\n", outFile)
  cat(str, "\n")
  invisible(dev.off())
...
#
# the function assumes that the dataset contains 2 biological conditions (control and treatment)
# and each with n replicates, ordered in the input files such that 1st col is Sym, then cols 2:n are for cond1 and (n+1):ncol for cond2.  
#
...
length_bias_analysis_compare_replicate_samples <- function(countsFile.Pgenes, Hs.Tx.LenFile,N.replicates=3, DatasetTitle="TNFa.p65kd", Rep.first.col=5, X.lim=c(8, 15), Y.lim=c(-3,3)) {
  ...
  # analyze the p65kd.tnfa replicates
  ...
  # Tests: comparisons of replicates 2 and 3 to replicate 1
  lrt <- glmLRT(fit, coef=2:N.replicates)
  
  # Results including log2FC are in lrt$table (2vs1 and 3vs1)
  head(lrt$table)
  FC.data <- list()
  m <- N.replicates-1
  ind <- 1
  ...
  edgeR_TMM_plots <- plot_replicate_samples(log2.Tx.Len,FC.data,"edgeR-TMM",DatasetTitle,rep_names,xLab="Tx.length (log2)", X.lim, Y.lim)
  ...
  RPKM_plots <- plot_replicate_samples(log2.Tx.Len,FC.data,"RPKM",DatasetTitle,rep_names,xLab="Tx.length (log2)", X.lim, Y.lim)
  ...
  RPKM.qnorm_plots <- plot_replicate_samples(log2.Tx.Len,FC.data,"RPKM.qnorm",DatasetTitle,rep_names,xLab="Tx.length (log2)", X.lim, Y.lim)
...
#
# the function assumes that the dataset contains 2 biological conditions (control and treatment)
# and each with 3 replicates, ordered in the input files such that 1st col is Sym, then cols 2:4 are for cond1 and 5:7 for cond2.  
#
...
length_bias_analysis_cqn_normalization <- function (countsFile.Pgenes, Hs.Tx.GC.Content.And.LenFile,N.replicates=3, DatasetTitle="TNFa.p65kd", Rep.first.col=5, X.lim=c(8, 15), Y.lim=c(-4,4)) {
  ...
  outFile <- sprintf("%s_RPKM_cqn_norm_log2.txt", DatasetTitle);
  write.table(CQN_norm,outFile,row.names = T,col.names = T,quote = F,sep="\t")
  outFile <- sprintf("%s_cqn_offset_log2.txt", DatasetTitle);
  write.table(offset_res,outFile,row.names = T,col.names = T,quote = F,sep="\t")
  ...
  FC.data <- FC_replicate_samples(2^CQN_norm_replicates)
  CQN.qnorm_plots <- plot_replicate_samples(log(counts$length,2),FC.data,"CQN_GC",DatasetTitle,rep_names,xLab="Tx.length (log2)", X.lim, Y.lim)
  for(i in seq(length(FC.data))){
    rep.p <- cor.test(log2.Tx.Len, FC.data[[i]], method="spearman")
    P.rep[2, i] <- rep.p$p.value; 
    Spearman.r.rep[2, i] <- rep.p$estimate; 
  ...
  FC.data <- FC_replicate_samples(2^CQN_norm_replicates)
  plot_replicate_samples(log(counts$length,2),FC.data,"CQN_GC_Length",DatasetTitle,rep_names,xLab="Tx.length (log2)", X.lim, Y.lim)
  invisible(dev.off())
  ...
  str <- sprintf("\nResults recorded in %s\n", outFile)
  cat(str, "\n")
  outFile <- sprintf("%s_cqn_replicate_samples_analysis_logFC.txt", DatasetTitle);
  FC <- do.call(cbind, FC.data)
  M.FC <- cbind(counts$sym,FC)
  write.table(M.FC, file=outFile, sep="\t", quote=F,row.names = T ,col.names=NA)
  str <- sprintf("\nlogFC results recorded in %s\n", outFile)
  cat(str, "\n")
  ...
  edgeR.TMM.logFC <- lrt$table[,1];  
  main.title <- sprintf("%s: edgeR-TMM", DatasetTitle);
  r <- plotFCvsLen(logTxLen=log2.Tx.Len, logFC=edgeR.TMM.logFC, main.title,xLab = "Tx.length (log2)", X.lim, Y.lim)
  P.bio[1, 1]          <- r$p.value; 
  Spearman.r.bio[1, 1] <- r$estimate; 
  
  colnames(Hs.genes.len.gc) <-  c("length","GC.content")
  # Examine relationship between FC and Tx length for biological conditions after CQN normalization with GC 
  cqn_res <- cqn(counts = counts.matrix,x = counts$GC.content,lengths = counts$length, lengthMethod = "fixed") # cqn normalization  
  CQN_norm <- cqn_res$y + cqn_res$offset # values are in log2 
  CQN.logFC <- calcLogFC(2^CQN_norm,Conds)
  main.title <- sprintf("%s: CQN_GC", DatasetTitle);
  r <- plotFCvsLen(logTxLen = log(counts$length,2),logFC = CQN.logFC, main.title = main.title ,xLab = "Tx.length (log2)", X.lim = X.lim, Y.lim = Y.lim)
  P.bio[1, 2]          <- r$p.value; 
  Spearman.r.bio[1, 2] <- r$estimate; 
  ...
  outFile <- sprintf("%s_cqn_treatment_vs_control_analysis_logFC.txt", DatasetTitle);
  write.table(CQN.logFC, file=outFile, sep="\t", quote=F,row.names = T ,col.names=NA)
  str <- sprintf("\nlogFC results recorded in %s\n", outFile)
  cat(str, "\n")
  
  c(res_replicates,res_bio)
}
